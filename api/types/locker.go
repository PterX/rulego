...
//
// Sources that already provide single delivery — Kafka consumer groups, MQTT
// shared subscriptions, load-balanced requests — must not add this guard on
// top: legitimate redeliveries would be rejected and at-least-once semantics
// would silently degrade to at-most-once.
//
...
// 否则各副本生成不同锁键，去重将静默失效。
//
// 已具备单投保证的触发源（Kafka 消费组、MQTT 共享订阅、负载均衡单播）
// 不应叠加本守卫：合法重投会被拒绝，at-least-once 语义会静默退化为
// at-most-once。
